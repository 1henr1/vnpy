...
void TdApi::processRspQryAccount(Task task) {
	...
	data["AccountTradeRight"] = task_data.AccountTradeRight;					///<交易状态
	data["CommodityGroupNo"] = task_data.CommodityGroupNo;						///<可交易品种组.
	data["AccountShortName"] = task_data.AccountShortName;                       ///< 账号简称
	data["AccountEnShortName"] = task_data.AccountEnShortName;						///<账号英文简称
...
void TdApi::processRspQryFund(Task task) {
	...
	data["AuthMoney"] = task_data.AuthMoney;                      ///< 授信资金
...
int TdApi::reqInsertOrder(dict req)
{
    ...
    getDouble(req, "RefDouble",&stNewOrder.RefDouble);						///<浮点参考值
    getStr(req, "RefString", stNewOrder.RefString);						///< 字符串参考值
    getStr(req, "ClientID", stNewOrder.ClientID);						///<客户子账号，如果存在子账号，则自行上报子账号
...
int TdApi::reqCancelOrder(dict req)
{
	...
	getDouble(req, "RefDouble", &stCancelReq.RefDouble);						///< 浮点参考值
...
int TdApi::reqAmendOrder(dict req)
{
    ...
    getStr(req, "AccountNo", stAmendOrder.ReqData.AccountNo);						///< 客户资金帐号，必填
    getStr(req, "ExchangeNo", stAmendOrder.ReqData.ExchangeNo);						///< 交易所编号，必填
    getChar(req, "CommodityType",&stAmendOrder.ReqData.CommodityType);					///< 品种类型，必填
    getStr(req, "CommodityNo", stAmendOrder.ReqData.CommodityNo);					///< 品种编码类型，必填
    getStr(req, "ContractNo", stAmendOrder.ReqData.ContractNo);						///< 合约1，必填
    getStr(req, "StrikePrice", stAmendOrder.ReqData.StrikePrice);					///< 执行价格1，期权填写
    getChar(req, "CallOrPutFlag",&stAmendOrder.ReqData.CallOrPutFlag);					///< 看张看跌1 默认N
    getStr(req, "ContractNo2", stAmendOrder.ReqData.ContractNo2);					///< 合约2，默认空
    getStr(req, "StrikePrice2", stAmendOrder.ReqData.StrikePrice2);					///< 执行价格2，默认空
    getChar(req, "CallOrPutFlag2",&stAmendOrder.ReqData.CallOrPutFlag2);					///< 看张看跌2 默认N
    getChar(req, "OrderType",&stAmendOrder.ReqData.OrderType);						///< 委托类型 必填
    getChar(req, "OrderSource",&stAmendOrder.ReqData.OrderSource);					///< 委托来源，默认程序单。
    getChar(req, "TimeInForce",&stAmendOrder.ReqData.TimeInForce);					///< 委托有效类型,默认当日有效
    getStr(req, "ExpireTime", stAmendOrder.ReqData.ExpireTime);						///< 有效日期(GTD情况下使用)
    getChar(req, "IsRiskOrder",&stAmendOrder.ReqData.IsRiskOrder);					///< 是否风险报单，默认非风险保单
    getChar(req, "OrderSide",&stAmendOrder.ReqData.OrderSide);						///< 买入卖出
    getChar(req, "PositionEffect",&stAmendOrder.ReqData.PositionEffect);					///< 开平标志1,默认N
    getChar(req, "PositionEffect2",&stAmendOrder.ReqData.PositionEffect2);				///< 开平标志2，默认N
    getStr(req, "InquiryNo", stAmendOrder.ReqData.InquiryNo);						///< 询价号
    getChar(req, "HedgeFlag",&stAmendOrder.ReqData.HedgeFlag);						///< 投机保值，默认N
    getDouble(req, "OrderPrice",&stAmendOrder.ReqData.OrderPrice);						///< 委托价格1
    getDouble(req, "OrderPrice2",&stAmendOrder.ReqData.OrderPrice2);					///< 委托价格2，做市商应价使用
    getDouble(req, "StopPrice",&stAmendOrder.ReqData.StopPrice);						///< 触发价格
    getUInt(req, "OrderQty",&stAmendOrder.ReqData.OrderQty);						///< 委托数量，必填
    getUInt(req, "OrderMinQty",&stAmendOrder.ReqData.OrderMinQty);					///< 最小成交量，默认1
    getUInt(req, "MinClipSize",&stAmendOrder.ReqData.MinClipSize);					///< 冰山单最小随机量
    getUInt(req, "MaxClipSize",&stAmendOrder.ReqData.MaxClipSize);					///< 冰山单最大随机量
    getInt(req, "RefInt",&stAmendOrder.ReqData.RefInt);							///< 整型参考值
    getDouble(req, "RefDouble",&stAmendOrder.ReqData.RefDouble);						///<浮点参考值
    getStr(req, "RefString", stAmendOrder.ReqData.RefString);						///< 字符串参考值
    getStr(req, "ClientID", stAmendOrder.ReqData.ClientID);						///<客户子账号，如果存在子账号，则自行上报子账号
    getChar(req, "TacticsType",&stAmendOrder.ReqData.TacticsType);					///< 策略单类型，默认N
    getChar(req, "TriggerCondition",&stAmendOrder.ReqData.TriggerCondition);				///< 触发条件，默认N
    getChar(req, "TriggerPriceType",&stAmendOrder.ReqData.TriggerPriceType);				///< 触发价格类型，默认N
    getChar(req, "AddOneIsValid",&stAmendOrder.ReqData.AddOneIsValid);					///< 是否T+1有效,默认T+1有效。
